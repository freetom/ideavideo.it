...
function fix_menu() {
    ...
    for (let i = 1; i <= 3; i++) {
        const cell = top_table.rows[0].cells[i].cloneNode(true);
    ...
    // Fix top table (where logo is) - or it will be too large for the display
    ...
    top_table.insertAfter(emptyRow, top_table.rows[0]);
    ...
    for (let i = 0; i < 3; i++) {
        ...
        cell.innerHTML = ' &nbsp;';
        row.appendChild(cell);
        top_table.insertBefore(row, top_table.rows[0]);
    }
}

function fix_page_footer() {
    getById('page-footer').className = '';
    getById('page-footer').style.marginLeft = '0';
}
